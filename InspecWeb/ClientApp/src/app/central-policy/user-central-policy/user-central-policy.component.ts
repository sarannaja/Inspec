    ...
    this.centralpolicyservice.acceptCentralpolicy(answer, this.id, this.userid)
      .subscribe(response => {
        ...
        for (let i = 0; i < CentralpolicyId.length; i++) {
          this.notificationService.addNotification(CentralpolicyId[i].centralPolicyId, this.provinceid, this.userid, 2, this.id)
            .subscribe(response => {
              console.log(response);
            })
        }
  ...
  sendAssignInternal(value) {
    ...
    this.centralpolicyservice.sendAssignInternal(value, this.id, this.userid).subscribe(response => {
      ...
      for (let i = 0; i < CentralpolicyId2.length; i++) {
        this.notificationService.addNotification(CentralpolicyId2[i].centralPolicyId, this.provinceid, value.UserId, 19, this.id)
          .subscribe(response => {
            console.log(response);
          })
      }
