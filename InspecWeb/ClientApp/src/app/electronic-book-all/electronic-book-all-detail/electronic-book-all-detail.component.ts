...
@Component({
  selector: 'app-electronic-book-all-detail',
  templateUrl: './electronic-book-all-detail.component.html',
  styleUrls: ['./electronic-book-all-detail.component.css']
})
export class ElectronicBookAllDetailComponent implements OnInit {
    ...
    this.electronicBookService.editSuggestion(value, this.electId).subscribe(res => {
      ...
      if (value.Status == "ใช้งานจริง") {
        ...
        value.user.forEach(element => {
          this.notificationService.addNotification(this.electronicBookData.electronicBookGroup[0].centralPolicyId, 1, element, 7, this.electId,null,this.userid)
            .subscribe(response => {
